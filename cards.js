...
var cards = (function () {
  ...
  Container.prototype.extend({
    ...
    lowestCard: function (suit) {
      if(this.length == 0) return null;
      if(suit != null) {
        const suitCards = this.filter(card => card.suit == suit);
        return suitCards.reduce((lowest, card) => card.rank < lowest.rank ? card : lowest, suitCards[0]);
      }
      return this.reduce((lowest, card) => card.rank < lowest.rank ? card : lowest, this[0]);
    },
    highestCard: function (suit) {
      if(this.length == 0) return null;
      if(suit != null) {
        const suitCards = this.filter(card => card.suit == suit);
        return suitCards.reduce((highest, card) => card.rank > highest.rank ? card : highest, suitCards[0]);
      }
      return this.reduce((highest, card) => card.rank > highest.rank ? card : highest, this[0]);
    },
    lowestQualifiedCard: function (qf) {
      if(this.length == 0) return null;
      if(qf != null) {
        const qualifiedCards = this.filter(qf);
        return qualifiedCards.reduce((lowest, card) => card.rank < lowest.rank ? card : lowest, qualifiedCards[0]);
      }
      return this.reduce((lowest, card) => card.rank < lowest.rank ? card : lowest, this[0]);
    },
    highestQualifiedCard: function (qf) {
      if(this.length == 0) return null;
      if(qf != null) {
        const qualifiedCards = this.filter(qf);
        return qualifiedCards.reduce((highest, card) => card.rank > highest.rank ? card : highest, qualifiedCards[0]);
      }
      return this.reduce((highest, card) => card.rank > highest.rank ? card : highest, this[0]);
    },
